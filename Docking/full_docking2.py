...
while True:
    ...
    if cX < frame.shape[1] / 2 - 20:
        ser.write(b'left\n')  # sending a left command to the microcontroller
    elif cX > frame.shape[1] / 2 + 20:
        ser.write(b'right\n')  # sending a right command to the microcontroller
    else:
        ser.write(b'forward\n')  # sending a forward command to the microcontroller

    # Find the height of the docking station
    height, width, channels = frame.shape
    distance = height - cY
    if distance < height / 2 - 20:
        ser.write(b'ascend\n')  # sending an ascend command to the microcontroller
    elif distance > height / 2 + 20:
        ser.write(b'descend\n')  # sending a descend command to the microcontroller
    else:
        ser.write(b'hold\n')  # sending a hold command to the microcontroller
